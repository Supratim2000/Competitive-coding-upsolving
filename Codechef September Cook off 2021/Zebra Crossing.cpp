...
ll lastIndx(string str,char ch)
{
    ll indx=str.find_last_of(ch);
    if(indx==string::npos)
        return -1;
    return indx;
}
...
ll solve()
{
    ...
    ll fChar=str[0];
    ll nextChar;
    fChar=='0'?nextChar='1':nextChar='0';
    for(ll i=0;i<n;i++)
    {
        if(str[i]==nextChar)
        {
            numAltr++;
            nextChar=='0'?nextChar='1':nextChar='0';
        }
    }
    if(k>numAltr)
        return -1;
    ...
    {
        if(k&1)
            return fChar=='0'?lastIndx(str,'1')+1:lastIndx(str,'0')+1;
        else
            return lastIndx(str,fChar)+1;
    }
}
